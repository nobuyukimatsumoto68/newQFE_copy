...
class QfeLattice {
 public:
  ...
  int AddFace(int a, int b, int c, Double wt);
  int AddFace(int a, int b, int c, int d, Double wt);
...
void QfeLattice::InitTriangle(int Nx, int Ny, Double wt1, Double wt2,
  ...
  for (int s = 0; s < n_sites; s++) {
    ...
    AddFace(s, xp1 + y * Nx, x + yp1 * Nx, 1.0);
    AddFace(xp1 + y * Nx, xp1 + yp1 * Nx, x + yp1 * Nx, 1.0);
  ...
  for (int f = 0; f < old_faces.size(); f++) {
    ...
    while (e_outer[0].size() >= 3) {
      ...
      for (int e = 0; e < 3; e++) {
        ...
        // corner triangle
        AddFace(e_outer[e][0], e_outer[e][1], s_a, 1.0);
        ...
        for (int i = 0; i < inner_size; i++) {
          ...
          AddFace(e_outer[e][i + 1], s, sm1, 1.0);
          AddFace(e_outer[e][i + 1], e_outer[e][i + 2], s, 1.0);
      ...
      if (inner_size == 0) {
        AddFace(e_outer[0][1], e_outer[1][1], e_outer[2][1], 1.0);
      } else if (inner_size == 2) {
        AddFace(e_inner[0][0], e_inner[1][0], e_inner[2][0], 1.0);
      }
