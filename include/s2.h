...
void QfeLatticeS2::ReadBaseLattice(int q) {
  ...
  // std::cout << lattice_path.c_str() << std::endl;
...
void QfeLatticeS2::WriteSite(FILE* file, int s) {
  ...
  Double theta = std::acos(r[s].z());
  // Double phi = atan2(r[s].y(), r[s].x());
  Double phi = std::atan2(r[s].y(), r[s].x());
  // fprintf(file, " %+.20f %+.20f", theta, phi);
  fprintf(file, " %+.30f %+.30f", theta, phi);
}
...
void QfeLatticeS2::ReadSite(FILE* file, int s) {
  ...
  r[s][0] = std::sin(theta) * std::cos(phi);
  r[s][1] = std::sin(theta) * std::sin(phi);
  r[s][2] = std::cos(theta);
  r[s].normalize();
  ...
  Vec3 north_pole(zero, zero, one);
  Vec3 south_pole(zero, zero, -one);
...
/// @brief Update positions of all sites ustd::sing orbits and symmetry group data
